...
//
// Defaults below target ProRanked production (api.proranked.com).
window.PRORANKED_CONFIG = {
  // ProRanked Identity (OAuth/OIDC issuer).
  oidcAuthority: 'https://id.proranked.com',
  ...
  oidcResource: 'https://api.proranked.com/cpms/v1',
  // Where the browser actually sends the calls (the CPO host serving /cpms/v1). Cross-origin → exercises
  // the SPA-bearer CORS policy. Empty string '' = same-origin (use the Vite dev proxy).
  cpmsApiBase: 'https://api.proranked.com',
};
